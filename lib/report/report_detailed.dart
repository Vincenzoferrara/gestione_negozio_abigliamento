...
class PerformanceTimelineReportPage extends StatefulWidget {
  ...
  const PerformanceTimelineReportPage({
    super.key,
    required this.periodo,
  });

  @override
  State<PerformanceTimelineReportPage> createState() => _PerformanceTimelineReportPageState();
}

class _PerformanceTimelineReportPageState extends State<PerformanceTimelineReportPage> {
  final ReportService _reportService = ReportService();
  ReportVenditeDettagliato? _reportVendite;
  bool _isLoading = false;
  String? _errorMessage;
  ...
  Widget _buildSummaryCard(String label, String value, IconData icon, int colorValue) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  Widget _buildDetailedStats() {
    ...
    final giornoMigliore = vendite.reduce((a, b) => a.totale > b.totale ? a : b);
    final giornoPeggiore = vendite.reduce((a, b) => a.totale < b.totale ? a : b);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            _buildStatRow('Media Giornaliera', ReportFormatter.formatCurrency(mediaGiornaliera)),
            const Divider(),
            _buildStatRow(
              'Giorno Migliore',
            ...
            const Divider(),
            _buildStatRow('Media Ordini/Giorno', (totaleOrdini / vendite.length).toStringAsFixed(1)),
          ],
        ),
      ),
    );
...
class OrdersDetailedReportPage extends StatefulWidget {
  ...
  @override
  State<OrdersDetailedReportPage> createState() => _OrdersDetailedReportPageState();
}
...
class _OrdersDetailedReportPageState extends State<OrdersDetailedReportPage> {
  ...
  Widget _buildReviewStats() {
    final totalReviews = _reviewStats!.values.fold(0, (sum, count) => sum + count);
  ...
  Widget _buildOrdersStatusList() {
    final totalOrders = _ordersByStatus!.values.fold(0, (sum, count) => sum + count);
