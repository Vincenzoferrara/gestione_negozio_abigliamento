...
class WooProductItemAttribute {
  ...
  ///
  /// NOTA (patch locale): i prodotti usano il formato `options` (lista),
  /// le varianti il formato `option` (stringa singola). Il fromJson
  /// originale chiamava `json['options'].cast<String>()` e falliva con
  /// NoSuchMethodError quando `options` era null (varianti). Mappiamo
  /// `option` → `options: [option]` per uniformare i due formati.
